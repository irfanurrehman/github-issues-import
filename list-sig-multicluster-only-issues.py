...
def send_request_to_path(which, full_url, post_data=None):
...
def get_all_sig_multicluster_issues(which):
    issues = []
    page = 1
    while True:
        new_list = send_request_to_path(which, "https://api.github.com/search/issues?q=is:issue+is:open+label:area/federation+repo:kubernetes/kubernetes&page=%d" % page)
        if not new_list['items']:
            break
        issues.extend(new_list['items'])
        page += 1
    return issues

def is_issue_only_for_multicluster(issue):
     for label in issue['labels']:
        label_name = label['name']
        if "sig/" in label_name:
            if label_name != "sig/multicluster":
                return False
...
def close_issue_by_comment(comment, old_issue_number, issue_number):
	comment['body'] = "/close \nThis issue was moved to kubernetes/federation#%s" % issue_number
...
if __name__ == '__main__':
    ...
    issues += get_all_sig_multicluster_issues('source')
    count = 0
    issue_list = " "
    for issue in issues:
        if is_issue_only_for_multicluster(issue):
            count += 1
            issue_list += " %s" % issue['number']
    print("total issues found", count)
    print(issue_list)
